...
export const getWeekArticles = ({year, week}) => request(Method.GET, `${API_ENDPOINT}/articles?year=${year}&week=${week}`)

export const getWeekConfig = ({week, year}) => request(Method.GET, `${API_ENDPOINT}/weeks?year=${year}&week=${week}`)
...
export const createWeek = (config) => axios.post(`${API_ENDPOINT}/weeks`, JSON.stringify(config))

export const createArticles = (articles) => axios.all(articles.map(async a => {
...
export const mjml2html = (mjml) => fetch(`${SOCKET_ENDPOINT}/mjml2html`, {
    method:'POST',
    headers:{'Content-Type':'text/html'},
    body:mjml
});

export const sendEmail = (mjml) => fetch(`${SOCKET_ENDPOINT}/send_email`, {
    method:'POST',
    headers:{'Content-Type':'text/html'},
    body:mjml
});
